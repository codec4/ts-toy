import { BehaviorSubject, merge, Observable, of, OperatorFunction, pairs } from "rxjs";
...
function lru<T extends Person = any>(project: Get<T>): OperatorFunction<string, T> {
  return function(source: Observable<string>): Observable<T> {
    return source.pipe<T>(switchMap((value => {
      return project(value).pipe(map((person: any) => {
